...
class Plugins
{
  public:
    ...
    __int64 GetPluginHandle(bool uninst, const tstring& command) const;
    tstring GetPluginPath(const tstring& command) const;
    tstring UseUnicodeVariant(const tstring& lcsig) const;
    void SetDllDataHandle(bool uninst, const tstring& command, __int64 dataHandle);
  ...
  private: // data members
    ...
    std::map<tstring, __int64> m_command_to_data_handle;
    std::map<tstring, __int64> m_command_to_uninstall_data_handle;
};
