...
#ifdef _WIN32
int MMapFile::setfile(HANDLE hFile, __int64 dwSize)
#else
...
__int64 MMapFile::getsize() const
{
  return m_iSize;
}

void *MMapFile::get(__int64 offset, int size) const
{
...
void *MMapFile::getmore(__int64 offset, int size) const
{
  void *pView;
...
void MMapFake::set(const char *pMem, __int64 iSize)
{
  m_pMem = pMem;
  m_iSize = iSize;
}

__int64 MMapFake::getsize() const
{
  return m_iSize;
}

void *MMapFake::get(__int64 offset, int size) const
{
  return get(offset, &size);
}
...
void *MMapFake::getmore(__int64 offset, int size) const
{
  return get(offset, size);
}

void MMapFake::resize(__int64 n) {}
...
__int64 MMapBuf::add(const void *data, __int64 len) 
{ 
  if (len <= 0) return 0;
  resize(getlen() + len);
...
void MMapBuf::resize(__int64 newlen)
{
...
void *MMapBuf::get(__int64 offset, int *sizep) const
{
  if (!sizep)
    return NULL;
  int size = *sizep;
...
void *MMapBuf::get(__int64 offset, int size) const
{
  if (m_gb_u)
...
void *MMapBuf::getmore(__int64 offset, int size) const
{
  if (m_gb_u)
