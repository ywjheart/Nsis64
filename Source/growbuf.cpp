...
__int64 GrowBuf::add(const void *data, __int64 len)
{
  if (len<=0) return 0;
  resize(m_used+len);
...
void GrowBuf::resize(__int64 newlen)
{
  __int64 os=m_alloc;  // old size
  __int64 ou=m_used;   // old used
  m_used=newlen;
  if (newlen > m_alloc)
  {
    void *n;
...
__int64 GrowBuf::getlen() const { return m_used; }
