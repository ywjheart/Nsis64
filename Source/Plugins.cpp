...
__int64 Plugins::GetPluginHandle(bool uninst, const tstring& command) const {
  if (uninst) {
    return get_value(m_command_to_uninstall_data_handle, command, (__int64)-1);
  }
  else {
    return get_value(m_command_to_data_handle, command, (__int64)-1);
  }
}
...
void Plugins::SetDllDataHandle(bool uninst, const tstring& command, __int64 dataHandle)
{
  if (uninst) {
