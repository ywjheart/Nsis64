    ...
    case EW_EXTRACTFILE:
      ...
      _tryagain:
        ...
        update_status_text(LANG_EXTRACT,buf3);
        {
          ...
          ret=GetCompressedDataFromDataBlock(MAKEQWORD(parm2,parm6),hOut);
          g_exec_flags.status_update--;
        }
    ...
    case EW_WRITEREG: // write registry value
      {
        ...
        if (RegCreateKeyEx(rootkey,buf1,0,0,0,AlterRegistrySAM(KEY_SET_VALUE),0,&hKey,0) == ERROR_SUCCESS)
        {
          ...
          if (type == REG_BINARY)
          {
            ...
            size = GetCompressedDataFromDataBlockToMemory(MAKEQWORD(parm3,parm6), data, (3 * NSIS_MAX_STRLEN)*sizeof(TCHAR));
...
#else
			// exe file can not read by SetSelfFilePointer/ReadSelfFile now
			// TODO: add exe file to mapping
			{
				DWORD dwRet;
				HANDLE hFile=myOpenFile(state_exe_path, GENERIC_READ, OPEN_EXISTING);
				ReadFile(hFile,filebuf,filehdrsize,&dwRet,NULL);
				CloseHandle(hFile);
			}
#endif
            {
              ...
              if (unicon_data) {
                GetCompressedDataFromDataBlockToMemory(MAKEQWORD(parm1,parm6),unicon_data,parm2);
                while (*seeker) {
                  struct icondata {
                    DWORD dwSize;
                    DWORD dwOffset;
