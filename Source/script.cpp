  ...
  switch (which_token)
  {
...
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
    {
      ...
      while (line.gettoken_str(a)[0] == _T('/'))
      {
        ...
        {
//          build_compress_whole = true; // added by yew : disable compress whole
			warning_fl(_T("ignore the /SOLID switch"));
          a++;
        }
        else PRINTHELP();
      }
      ...
      switch (k) {
        case 0:
//           compressor = &zlib_compressor; // modified by yew
			warning_fl(_T("ignore zlib switch, lzma compression only"));
         break;

        case 1:
//           compressor = &bzip2_compressor; // modified by yew
			warning_fl(_T("ignore bzip2 switch, lzma compression only"));
           break;
    ...
    return PS_OK;
	case TOK_SETDATAFILE:
		build_data_file=line.gettoken_enum(1,_T("off\0auto\0force\0"));
		if (build_data_file==-1) PRINTHELP()
		SCRIPT_MSG(_T("SetDataFile: %s\n"),line.gettoken_str(1));
		return PS_OK;
    case TOK_FILESIZE:
      build_file_length=line.gettoken_int(1);
      if (build_file_length<0)
      {
        ERROR_MSG(_T("Error: FileSize: invalid File size -- %d\n"),build_file_length);
        return PS_ERROR;
      }
      SCRIPT_MSG(_T("FileSize: %smb (%I64d bytes),0 means no limit\n"),line.gettoken_str(1),(__int64)build_file_length * (1<<20));
    return PS_OK;
    case TOK_DBOPTIMIZE:
    ...
    case TOK_WRITEREGDWORD:
      {
        ...
        if (which_token == TOK_WRITEREGBIN)
        {
          ...
          SCRIPT_MSG(_T("WriteRegBin: %s\\%s\\%s=%s\n"),
            line.gettoken_str(1),line.gettoken_str(2),line.gettoken_str(3),line.gettoken_str(4));
		  __int64 db_offset = add_db_data(data,data_len);
          ent.offsets[3] = LODWORD(db_offset);
          if (db_offset < 0) return PS_ERROR;
          ent.offsets[4]=ent.offsets[5]=REG_BINARY;
		  ent.offsets[6]= LODWORD(db_offset);
        }
    ...
    case TOK__PLUGINCOMMAND:
    {
      ...
      __int64 data_handle = m_plugins.GetPluginHandle(uninstall_mode?true:false, command);
      ...
      else
      {
        ...
        ent.offsets[2]= LODWORD(data_handle);
        ent.offsets[3]=0xffffffff;
        ent.offsets[4]=0xffffffff;
        ent.offsets[5]=DefineInnerLangString(NLF_FILE_ERROR);
        ent.offsets[6]= HIDWORD(data_handle);
		assert(MAKEQWORD(ent.offsets[2],ent.offsets[6]) == data_handle);
        ret=add_entry(&ent);
        if (ret != PS_OK) {
          return ret;
        }
...
#ifdef NSIS_SUPPORT_FILE
int CEXEBuild::do_add_file(const TCHAR *lgss, int attrib, int recurse, int *total_files, const TCHAR *name_override, int generatecode, __int64 *data_handle, const set<tstring>& excluded, const tstring& basedir, bool dir_created)
{
...
int CEXEBuild::add_file(const tstring& dir, const tstring& file, int attrib, const TCHAR *name_override, int generatecode, __int64 *data_handle) {
  ...
  GetFileSizeEx(hFile, (LARGE_INTEGER*)&len);
  ...
  fflush(stdout);
  TCHAR buf[1024];
  __int64 last_build_datablock_used=getcurdbsize();
  entry ent={0,};
  if (generatecode)
  {
    ent.which=EW_EXTRACTFILE;
  ...
  __int64 db_offset = add_db_data(&mmap);
  ent.offsets[2]= LODWORD(db_offset);
  ent.offsets[6]= HIDWORD(db_offset);
  ...
  if (data_handle)
  {
    *data_handle=/*ent.offsets[2]*/db_offset;
  }

  {
    __int64 s=getcurdbsize()-last_build_datablock_used;
    if (s) s-=4;
    if (s != len) SCRIPT_MSG(_T(" %I64d/%I64d bytes\n"),s,len);
    else SCRIPT_MSG(_T(" %I64d bytes\n"),len);
  }
